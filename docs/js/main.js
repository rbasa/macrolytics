...
// Find the first date with UVA data
async function findFirstUVADate() {
    const query = `
        SELECT MIN(DATE) as first_date
        FROM fx_rate
        WHERE pair = 'UVA_ARS' AND kind = 'index'
    `;
    try {
        const result = await fetchDoltHubData(query);
        if (result.length > 0 && result[0].first_date) {
            return result[0].first_date;
        }
    } catch (error) {
        console.warn('Could not find first UVA date, using default:', error);
    }
    return '2016-03-31'; // fallback
}
